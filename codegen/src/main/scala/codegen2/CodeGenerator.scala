...
object CodeGenerator {
  ...
  def run(profile: String, jdbcDriver: String, url: String, outputDir: String, pkg: String, user: Option[String], password: Option[String], ignoreInvalidDefaults: Boolean, codeGeneratorClass: Option[String],
    ...
    val tables = if (includedTables.isDefined && !includedTables.get.isEmpty) {
      Some(profileInstance.defaultTables.map(_.filter(includedTables.get contains _.name.name)))
    } else if (excludedTables.isDefined && !excludedTables.get.isEmpty) {
      Some(profileInstance.defaultTables.map(_.filterNot(excludedTables.get contains _.name.name)))
    } else None

//    val tables = profileInstance.defaultTables
//      .map(ts => ts.filter(t => includedTables.isEmpty || (includedTables contains t.name.name)))
//      .map(ts => ts.filterNot(t => excludedTables contains t.name.name))
